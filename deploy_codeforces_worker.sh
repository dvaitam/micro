...
sudo nerdctl build -t "${IMAGE_NAME}" "${SCRIPT_DIR}/codeforces-worker"
...
kubectl set env deployment codeforces-worker DB_DSN="${POSTGRES_DSN}" --overwrite
kubectl set env deployment codeforces-worker KAFKA_BROKERS="${KAFKA_BROKERS}" --overwrite

log "Restarting deployment to pick up the new image"
kubectl rollout restart deployment codeforces-worker
kubectl rollout status deployment codeforces-worker --watch=false
